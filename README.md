...
The ESP32-C3 firmware acts as a transparent byte-stream bridge between the GNSS UART and both a BLE client (phone/tablet) and a TCP client. It uses the Nordic UART Service (NUS) for BLE and FreeRTOS StreamBuffers as ring buffers to decouple producer/consumer timing.
...
GNSS UART TX -> ESP32 UART RX -> [UART->TCP StreamBuffer] -> TCP socket -> Client
Client TCP WRITE -> [TCP->UART StreamBuffer] -> ESP32 UART TX -> GNSS UART RX
```
