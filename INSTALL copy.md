...
- A prebuilt binary set (`bootloader.bin`, `partitions.bin`, `firmware.bin`) provided in releases
...
Commands: see /build (full automation in a container). See /bin for the binaries and the flash commands
...
- Develop and validate UI/API behavior with `render-web`.
